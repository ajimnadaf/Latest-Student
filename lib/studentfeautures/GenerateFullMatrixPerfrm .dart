...
class GenerateFullMatrixPerfrm extends StatefulWidget {
  final bool clearanceIntent;
  const GenerateFullMatrixPerfrm({super.key, this.clearanceIntent = false});

  @override
  State<GenerateFullMatrixPerfrm> createState() =>
      _GenerateFullMatrixPerfrmState();
}

class _GenerateFullMatrixPerfrmState extends State<GenerateFullMatrixPerfrm>
...
""";
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false, // block system from killing activity
      onPopInvoked: (didPop) async {
        if (didPop) return;

        // 1️⃣ WebView internal navigation
        if (await _controller.canGoBack()) {
          await _controller.goBack();
          return;
        }

        // 2️⃣ SAME AS Java finish()
